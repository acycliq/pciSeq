...
class InputValidator:
    ...
    def validate(
        ...
        # Validate spots
        InputValidator._validate_spots(spots)
        spots = InputValidator._process_spots(spots.copy(), scdata, config)

        # Validate coo matrix
        InputValidator._validate_coo(coo)

        # Validate single cell data if present
        if scdata is not None:
            InputValidator._validate_scdata(scdata)

        # Validate config
        config = InputValidator._validate_config(config)
    ...
    def _validate_spots(spots: pd.DataFrame) -> None:
        ...
        if set(spots.columns) != {'gene_name', 'x', 'y', 'z_plane'}:
            raise ValueError("Spots dataframe must have columns ['gene_name', 'x', 'y', 'z_plane']")
